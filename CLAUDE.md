...
- **Recent Major Updates:**
  ...
  - v0.14.2 - Improved report parameters: insights_top_features & radar_max_features separate, feature importance sorting
...
3. **Dimensionality Reduction** - полностью готово (v0.1.0, v0.15.0)
   ...
   - **NEW v0.15.0:** Flexible dim_reduction parameter в Pipeline:
     - 'auto' - умный выбор PCA/UMAP/None based on algorithm + n_features
     - 'pca' - linear reduction (best for K-Means/GMM)
     - 'umap' - non-linear reduction для кластеризации (best for HDBSCAN/DBSCAN)
     - 'none' - работа в исходном пространстве
   - **ВАЖНО:** UMAP для кластеризации НЕ то же самое что UMAP для viz!
     - Кластеризация: n_components=10-20 (сохраняет структуру)
     - Визуализация: n_components=2-3 (только картинка)
...
### 1. PCA vs UMAP для кластеризации (обновлено v0.15.0)

**СТАРОЕ мнение (до v0.15.0):** t-SNE и UMAP ТОЛЬКО для визуализации!

**НОВОЕ понимание (v0.15.0+):** UMAP МОЖНО использовать для кластеризации при грамотной настройке!

#### Когда использовать PCA:
- ✅ K-Means, GMM, Hierarchical (distance-based algorithms)
- ✅ Сохраняет **глобальную** структуру
- ✅ Линейная трансформация → интерпретируемые компоненты
- ❌ Разрушает **локальную** плотность → плохо для DBSCAN/HDBSCAN

#### Когда использовать UMAP:
- ✅ DBSCAN, HDBSCAN (density-based algorithms)
- ✅ Сохраняет **локальную** плотность
- ✅ Manifold learning → выявляет скрытые структуры
- ❌ Нелинейная трансформация → нет интерпретации компонент

#### КРИТИЧНО: Разные настройки для viz vs clustering!

```python
# ❌ НЕПРАВИЛЬНО - для визуализации:
umap_viz = UMAP(n_components=2, min_dist=0.5)  # Только картинка!

# ✅ ПРАВИЛЬНО - для кластеризации:
umap_cluster = UMAP(
    n_components=10,       # НЕ 2! Сохраняем структуру
    n_neighbors=30,        # Размер локального соседства
    min_dist=0.1,          # Плотность кластеров
    random_state=42        # Reproducibility!
)
```

#### Auto-mode логика (v0.15.0):
- K-Means/GMM + <50 features → **none** (не нужен reduction)
- K-Means/GMM + ≥50 features → **pca** (сохраняет глобальную структуру)
- HDBSCAN/DBSCAN + <30 features → **none** (сохраняет локальную плотность)
- HDBSCAN/DBSCAN + ≥30 features → **umap** (manifold learning)

#### Почему это работает:
- UMAP с `random_state` фиксирован → стабильные результаты
- `n_components=10-20` → достаточно информации для кластеризации
- Сохраняет локальную плотность → HDBSCAN видит кластеры!
